...
export function buildAtomEntries(input: FeedInput, siteUrl: string, maxItems: number): FeedEntry[] {
  ...
  const sorted = withUpdated.toSorted((a, b) => {
    const au = a.item.frontmatter.updated ?? "";
    const bu = b.item.frontmatter.updated ?? "";
    if (au === bu) return a.item.slug.localeCompare(b.item.slug);
    return au < bu ? 1 : -1;
  });
  const truncated = sorted.slice(0, maxItems);
  return truncated.map(({ type, item }) => {
    const href = joinSiteUrl(siteUrl, `/${ROUTE_PREFIX[type]}/${item.slug}`);
    return {
      id: href,
      title: item.title,
