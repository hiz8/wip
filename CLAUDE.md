...
コメントは「正確で簡潔」に保つ。コードの動作をなぞるのではなく、読者がコードだけでは得られない情報を補う。

- **原則**: コードは「どう動くか」を語り、コメントは「なぜそうするか」を語る。動作のそのままの言い換えは書かない
- 自明なコメントは書かない。良い名前で代替できるならコメントは不要
- 簡潔に書く。ここでの簡潔さは冗長さ (言い直し・自明な説明) の排除であり、行数の削減ではない。1 つの概念を複数文で言い直さない
- あいまいな指示語 (「これ」「それ」「この処理」) を避け、具体的な主語・対象を書く
- 情報密度の高い、正確な語を選ぶ。冗長な言い換えよりドメイン語や具体例を使い、かつ読みやすさは保つ
- 次のものは複数行でも残す: 外部ツール/フォーマットの癖、プラットフォーム固有の gotcha、非自明な制約・トレードオフ、lint disable の理由
